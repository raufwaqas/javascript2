...
let JSON_API = JSON.parse(fetchPokemonData());
let sprites = Object.values(JSON_API.sprites).filter((x) => x);

console.log(JSON_API);

let id = (id) => document.getElementById(id),
  Img_element = id("sprite-img"),
  prev = id("prev"),
  next = id("next"),
  info = id("info"),
  name = id("name");
...
name.innerHTML = JSON_API.name;

Img_element.src = sprites[0];

next.addEventListener("mouseover", () => (next.src = "./img/next_hover.png"));
next.addEventListener("mouseout", () => (next.src = "./img/next.png"));

prev.addEventListener("mouseover", () => (prev.src = "./img/prev_hover.png"));
prev.addEventListener("mouseout", () => (prev.src = "./img/prev.png"));

next.addEventListener("click", () => {
  if (i == 3) {
    i = 0;
    Img_element.src = sprites[i];
  } else {
    i++;
    Img_element.src = sprites[i];
  }
